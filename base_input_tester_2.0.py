...
Version 2.0 Updates:
- CRITICAL: Added comprehensive window bounds safety validation
- Added thread-safe window operations with locking mechanisms
- Implemented window validity verification before all operations
- Enhanced coordinate validation to prevent out-of-bounds writes
- Added defensive programming patterns throughout
- Centralized validation logic for maintainability
- Improved error recovery for window-related operations
- Added detailed safety logging for debugging
"""
...
class WindowBoundsValidator:
    """
    Utility class for validating window bounds and coordinates.

    Provides centralized validation logic to ensure all window operations
    are performed within safe boundaries and with valid window handles.
    """

    @staticmethod
    def is_window_valid(hwnd):
        """
        Verify that a window handle is valid and the window still exists.

        Args:
            hwnd (int): Window handle to validate.
        ...
        try:
            # Check if window exists using win32gui
            return win32gui.IsWindow(hwnd)
        except Exception:
            return False

    @staticmethod
    def validate_coordinates(x, y, max_width, max_height):
        """
        ...
        Args:
            x (int): X coordinate.
            y (int): Y coordinate.
            max_width (int): Maximum width (exclusive).
            max_height (int): Maximum height (exclusive).

        Returns:
            tuple: Validated (x, y) coordinates guaranteed to be within bounds.
        """
        # Ensure coordinates are integers
        x = int(x)
        y = int(y)

        # Clamp to safe bounds (0 to max-1)
    ...
    @staticmethod
    def validate_coordinate_single(coord, max_value):
        """
        Validate and clamp a single coordinate value.

        Args:
            coord (int): Coordinate value.
            max_value (int): Maximum value (exclusive).

        Returns:
            int: Validated coordinate guaranteed to be within bounds.
        """
        coord = int(coord)
        return max(0, min(max_value - 1, coord))
...
class BaseInputTester:
    ...
    - Thread-safe window operations using locks
    - Window handle validation before all operations
    - Coordinate bounds validation
    - Defensive error handling
    - Automatic recovery from window errors
    ...
    Attributes:
        ...
        window_lock (threading.RLock): Reentrant lock for thread-safe window operations.
        bounds_validator (WindowBoundsValidator): Validator for window bounds checking.
    """
    ...
    def __init__(self, config_file=None):
        ...
        # Thread safety: Use RLock to allow same thread to acquire lock multiple times
        self.window_lock = threading.RLock()

        # Bounds validator instance
        self.bounds_validator = WindowBoundsValidator()
    ...
    def setup_logging(self):
        ...
        self.logger.info(f"Starting new {self.__class__.__name__} session (v2.0 - Enhanced Safety)")
        self.logger.info(f"Log file: {os.path.abspath(log_filename)}")
        self.logger.info(f"Configuration: {self.config}")
        self.logger.info("Safety features enabled: window validation, bounds checking, thread safety")

    def is_window_valid(self):
        """
        Check if the test window is valid and ready for operations.

        This method provides thread-safe validation of the window handle.

        Returns:
            bool: True if window is valid, False otherwise.
        """
        with self.window_lock:
            return self.bounds_validator.is_window_valid(self.test_window)
    ...
    def process_messages(self):
        ...
        SAFETY: Thread-safe and validates window before processing.
        """
        with self.window_lock:
            if not self.is_window_valid():
                self.logger.debug("Skipping message processing - window not valid")
                return
            ...
            try:
                ...
                while win32gui.PeekMessage(msg, self.test_window, 0, 0, win32con.PM_REMOVE):
                    ...
                    if message_count > 1000:
                        self.logger.warning("Message queue processing limit reached (1000 messages)")
                        break
    ...
    def cleanup_window(self):
        ...
        SAFETY: Thread-safe operation with proper validation.
        """
        with self.window_lock:
            self.logger.info("Performing window cleanup...")
            if self.test_window:
                try:
                    if self.bounds_validator.is_window_valid(self.test_window):
                        win32gui.DestroyWindow(self.test_window)
                except Exception as e:
                    self.logger.error(f"Error destroying window during cleanup: {e}")
                finally:
                    self.test_window = None
            ...
            try:
                ...
                self.last_cleanup_time = time.time()
                self.logger.info("Window cleanup completed successfully")
            except Exception as e:
            ...
            with self.window_lock:
                if self.test_window:
                    try:
                        ...
                        self.logger.error(f"Error destroying window in context exit: {e}")
                    finally:
                        self.test_window = None
    ...
    def start_testing(self, min_interval=None, max_interval=None):
        ...
        def testing_loop():
            ...
            SAFETY: Enhanced error recovery and window validation.
            """
            with self.test_window_context():
                ...
                while self.running:
                    try:
                        # Verify window is still valid before simulating event
                        if not self.is_window_valid():
                            self.logger.warning("Window became invalid, attempting to recreate...")
                            with self.window_lock:
                                self.create_test_window()
                        ...
                        self.logger.error(f"Error in testing loop (count: {consecutive_errors}): {e}")

                        # If too many consecutive errors, stop testing
                        if consecutive_errors >= max_consecutive_errors:
                            self.logger.critical(f"Too many consecutive errors ({consecutive_errors}), stopping testing")
                            self.running = False
                            break
                        ...
                        with self.window_lock:
                            if not self.is_window_valid():
                                try:
                                    ...
                                    self.logger.error(f"Failed to recreate window: {window_error}")
