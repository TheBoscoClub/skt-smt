...
class BaseInputTester:
    ...
    def load_config(self, config_file):
        ...
        if config_file:
            ...
            # Then try with .json extension if not found
            elif os.path.exists(f"{config_file}.json"):
                file_path = f"{config_file}.json"
            else:
                self._print_message(f"Config file not found: {config_file}")
            ...
            try:
                ...
                self._print_message(f"Successfully loaded config from: {file_path}")
                return merged_config
            except json.JSONDecodeError as e:
                self._print_message(f"Error parsing config file: {e}")
                return default_config
            except Exception as e:
                self._print_message(f"Error loading config file: {e}")
                return default_config
    ...
    def setup_logging(self):
        ...
        logging.info(f"Starting new {self.__class__.__name__} session (v1.7)")
