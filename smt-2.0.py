...
"""
SafeMouseTester v2.0 - An advanced utility for testing mouse input in an isolated environment.
...
New in v2.0:
- CRITICAL: Comprehensive window bounds safety validation
- Pre-validation of all coordinates before calculations
- Window validity checks before every operation
- Thread-safe coordinate operations
- Defensive coordinate clamping at multiple layers
- Enhanced error handling for window operations
- Centralized bounds validation logic
- Protection against writing outside window bounds
- Fail-safe mechanisms for coordinate overflow
...
class SafeMouseTester(BaseInputTester):
    ...
    - Comprehensive coordinate validation before all operations
    - Window validity verification before message sending
    ...
    def __init__(self, config_file="smt.config"):
        ...
        try:
            self.screen_width = max(1, win32api.GetSystemMetrics(0))
            self.screen_height = max(1, win32api.GetSystemMetrics(1))
        ...
        # Set default starting position to center of screen with validation
        self.current_x = self.bounds_validator.validate_coordinate_single(
            self.screen_width // 2,
            self.screen_width
        )
        self.current_y = self.bounds_validator.validate_coordinate_single(
        ...
        self.movement_min_distance = max(1, self.config.get("movement_min_distance", 10))
        self.movement_max_distance = max(self.movement_min_distance, self.config.get("movement_max_distance", 100))
        self.click_probability = max(0.0, min(1.0, self.config.get("click_probability", 0.2)))
        self.scroll_probability = max(0.0, min(1.0, self.config.get("scroll_probability", 0.1)))
        self.double_click_probability = max(0.0, min(1.0, self.config.get("double_click_probability", 0.05)))
        ...
        self.circular_min_radius = max(1, self.config.get("circular_min_radius", 20))
        self.circular_max_radius = max(self.circular_min_radius, self.config.get("circular_max_radius", 150))
        self.circular_min_steps = max(1, self.config.get("circular_min_steps", 8))
        self.circular_max_steps = max(self.circular_min_steps, self.config.get("circular_max_steps", 24))

        # Linear movement parameters
        self.linear_min_steps = max(1, self.config.get("linear_min_steps", 5))
        self.linear_max_steps = max(self.linear_min_steps, self.config.get("linear_max_steps", 20))

        self.logger.info("SafeMouseTester initialized with enhanced safety features")

    def validate_and_clamp_coordinates(self, x, y):
        """
        Validate and clamp coordinates to safe screen bounds.

        This is a critical safety method that ensures coordinates are always
        within valid screen boundaries before being used in any operation.
        ...
        Returns:
            tuple: Validated (x, y) coordinates guaranteed to be safe.
        """
        return self.bounds_validator.validate_coordinates(
    ...
    def create_test_window(self):
        ...
        SAFETY: Thread-safe window creation with proper error handling.
        """
        ...
        wc.lpszClassName = "IsolatedMouseTesterV2"
        ...
        self.logger.info("Window bounds safety validation enabled")
    ...
    def simulate_mouse_move(self, to_x, to_y):
        ...
        1. Validate window exists and is valid
        2. Validate and clamp coordinates before use
        3. Verify coordinates after clamping
        ...
        with self.window_lock:
            # SAFETY CHECK 1: Verify window is valid
            if not self.is_window_valid():
                self.logger.warning("Cannot simulate mouse move - window not valid")
                return False

            try:
                # SAFETY CHECK 2: Validate and clamp coordinates BEFORE use
                to_x, to_y = self.validate_and_clamp_coordinates(to_x, to_y)

                # SAFETY CHECK 3: Double-check coordinates are in valid range
                if not (0 <= to_x < self.screen_width and 0 <= to_y < self.screen_height):
                    self.logger.error(f"Coordinate validation failed: ({to_x}, {to_y}) outside bounds")
                    return False

                # Pack coordinates into lparam (low-order word has x, high-order word has y)
                # Additional safety: mask to ensure values fit in 16 bits
                lparam = ((to_y & 0xFFFF) << 16) | (to_x & 0xFFFF)

                # Send mouse move message
                win32gui.PostMessage(self.hidden_window, WM_MOUSEMOVE, 0, lparam)

                # Update current position (already validated)
                self.current_x = to_x
                self.current_y = to_y
    ...
    def simulate_mouse_click(self, button_type="left", double_click=False):
        ...
        SAFETY: Validates window and coordinates before sending messages.
        ...
        """
        with self.window_lock:
            # SAFETY CHECK: Verify window is valid
            if not self.is_window_valid():
                self.logger.warning("Cannot simulate mouse click - window not valid")
                return False

            try:
                # SAFETY CHECK: Validate current position
                self.current_x, self.current_y = self.validate_and_clamp_coordinates(
                    self.current_x, self.current_y
                )

                # Pack coordinates into lparam with safety masking
                lparam = ((self.current_y & 0xFFFF) << 16) | (self.current_x & 0xFFFF)
    ...
    def simulate_mouse_scroll(self, delta=120):
        ...
        SAFETY: Validates window and coordinates before sending.
        ...
        with self.window_lock:
            # SAFETY CHECK: Verify window is valid
            if not self.is_window_valid():
                self.logger.warning("Cannot simulate mouse scroll - window not valid")
                return False
    ...
    def simulate_random_movement(self):
        ...
        SAFETY: Validates coordinates before and after calculation.

        Returns:
            bool: True if the movement was simulated, False otherwise.
        """
        # SAFETY: Validate current position first
        self.current_x, self.current_y = self.validate_and_clamp_coordinates(
            self.current_x, self.current_y
        )
        ...
        if distance < self.movement_min_distance and distance > 0:
        ...
        # Calculate new position - will be validated in simulate_mouse_move
        new_x = self.current_x + delta_x
        new_y = self.current_y + delta_y

        # SAFETY: Validate before passing to simulate_mouse_move
        new_x, new_y = self.validate_and_clamp_coordinates(new_x, new_y)

        # Simulate the movement (has its own validation)
        if self.simulate_mouse_move(new_x, new_y):
            self.logger.info(f"Event {self.event_count}: Mouse moved randomly to ({new_x}, {new_y})")
            return True
        return False
    ...
    def simulate_linear_movement(self):
        ...
        SAFETY: Validates coordinates at each step.

        Returns:
            bool: True if the movement was simulated successfully, False otherwise.
        """
        # SAFETY: Validate current position
        self.current_x, self.current_y = self.validate_and_clamp_coordinates(
            self.current_x, self.current_y
        )
        ...
        end_x = self.current_x + delta_x
        end_y = self.current_y + delta_y

        # SAFETY: Validate end position
        end_x, end_y = self.validate_and_clamp_coordinates(end_x, end_y)
        ...
        for step in range(1, steps + 1):
            ...
            # SAFETY: Validate step position
            step_x, step_y = self.validate_and_clamp_coordinates(step_x, step_y)
    ...
    def simulate_circular_movement(self):
        ...
        SAFETY: Validates coordinates for center and each point on circle.
        ...
        # SAFETY: Ensure circle stays within screen bounds
        # Calculate safe center position
        center_x = max(radius, min(self.screen_width - radius - 1, self.current_x))
        center_y = max(radius, min(self.screen_height - radius - 1, self.current_y))

        # SAFETY: Validate center position
        center_x, center_y = self.validate_and_clamp_coordinates(center_x, center_y)
        ...
        for step in range(steps):
            ...
            # SAFETY: Validate circle point (critical - ensure we stay in bounds)
            x, y = self.validate_and_clamp_coordinates(x, y)
    ...
    def simulate_targeted_movement(self):
        ...
        SAFETY: Validates target coordinates and path coordinates.
        ...
        for target in self.targeted_targets:
            # SAFETY: Validate target ratios are in range [0, 1]
            x_ratio = max(0.0, min(1.0, target.get("x_ratio", 0.5)))
            y_ratio = max(0.0, min(1.0, target.get("y_ratio", 0.5)))

            target_x = int(x_ratio * self.screen_width)
            target_y = int(y_ratio * self.screen_height)

            # SAFETY: Validate target coordinates
            target_x, target_y = self.validate_and_clamp_coordinates(target_x, target_y)

            targets.append((target_x, target_y))
            weights.append(max(0, target.get("weight", 1)))
        ...
        curve_offset = max(0, int(distance * 0.1))  # 10% of distance
        ...
        # SAFETY: Validate midpoint
        midpoint_x, midpoint_y = self.validate_and_clamp_coordinates(
        ...
        for step in range(1, steps + 1):
            ...
            # SAFETY: Validate curve point (critical)
            x, y = self.validate_and_clamp_coordinates(x, y)
    ...
    def cleanup_window(self):
        ...
        SAFETY: Thread-safe with window validation.
        """
        with self.window_lock:
            self.logger.info("Performing window cleanup...")
            if self.hidden_window:
                try:
                    if self.bounds_validator.is_window_valid(self.hidden_window):
            ...
            try:
                ...
                self.logger.error(f"Error creating window during cleanup: {e}")
                raise
...
if __name__ == "__main__":
    ...
    print(f"SafeMouseTester v2.0 - Test mouse input in an isolated environment")
    print(f"Enhanced with comprehensive window bounds safety validation")
