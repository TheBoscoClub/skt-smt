...
"""
SafeKeyboardTester v2.0 - An advanced utility for testing keyboard input in an isolated environment.
...
- Window validity checks before every keypress
- Thread-safe keyboard operations
- Enhanced virtual key code validation
- Defensive error handling for all key operations
- Protection against sending events to invalid windows
...
class SafeKeyboardTester(BaseInputTester):
    ...
    - Window validity verification before all key operations
    - Virtual key code validation
    - Thread-safe keypress operations
    - Defensive error handling
    - Safe window cleanup
    - Protection against invalid window handles
    ...
    def __init__(self, config_file="skt.config"):
        ...
        # Cache frequently used config values with validation
        self.key_interval_min = max(0.0, self.config.get("key_interval_min", 0.1))
        self.key_interval_max = max(self.key_interval_min, self.config.get("key_interval_max", 0.3))
        self.word_length_min = max(1, self.config.get("word_length_min", 3))
        self.word_length_max = max(self.word_length_min, self.config.get("word_length_max", 8))
        self.typo_probability = max(0.0, min(1.0, self.config.get("typo_probability", 0.05)))
        self.correction_probability = max(0.0, min(1.0, self.config.get("correction_probability", 0.8)))
        self.capitalization_probability = max(0.0, min(1.0, self.config.get("capitalization_probability", 0.2)))
        self.common_words_probability = max(0.0, min(1.0, self.config.get("common_words_probability", 0.7)))
        self.special_key_probability = max(0.0, min(1.0, self.config.get("special_key_probability", 0.05)))
        self.space_after_word_probability = max(0.0, min(1.0, self.config.get("space_after_word_probability", 0.9)))

        self.logger.info("SafeKeyboardTester initialized with enhanced safety features")

    def validate_vk_code(self, vk_code):
        """
        Validate a virtual key code is within acceptable range.

        SAFETY: Ensures key codes are valid before use.

        Args:
            vk_code (int): Virtual key code to validate.
        ...
        if vk_code is None:
            return False
        # Valid VK codes are typically 0-255
        return 0 <= vk_code <= 255 and vk_code != -1
    ...
    def create_test_window(self):
        ...
        SAFETY: Thread-safe window creation with validation.
        """
        ...
        wc.lpszClassName = "IsolatedKeyboardTesterV2"
    ...
    def simulate_keypress(self, vk_code, char=None):
        ...
        1. Verify window is valid
        2. Validate virtual key code
        ...
        with self.window_lock:
            ...
            if not self.is_window_valid():
                self.logger.warning("Cannot simulate keypress - window not valid")
                return False

            # SAFETY CHECK 2: Validate virtual key code
            if not self.validate_vk_code(vk_code):
                self.logger.warning(f"Invalid virtual key code: {vk_code}")
                return False
            ...
            try:
                ...
                if char:
                    # SAFETY: Validate character can be converted to ord
                    try:
                        char_code = ord(char)
                        win32gui.PostMessage(self.transparent_window, WM_CHAR, char_code, 0)
                    except (TypeError, ValueError) as e:
                        self.logger.warning(f"Cannot convert character to ord: {char}, error: {e}")
    ...
    def simulate_common_word(self):
        ...
        SAFETY: Validates window before each keypress.
        ...
        for i, char in enumerate(word):
            # SAFETY: Verify window is still valid before each key
            if not self.is_window_valid():
                self.logger.warning("Window became invalid during typing")
                return False
            ...
            if random.random() < self.typo_probability:
                ...
                if self.validate_vk_code(vk_code) and self.simulate_keypress(vk_code, typo):
                    ...
                    if random.random() < self.correction_probability:
                        ...
                        if self.simulate_keypress(win32con.VK_BACK):
                            typed_chars.pop() if typed_chars else None  # Remove the typo safely
                            char_delay = random.uniform(self.key_interval_min, self.key_interval_max)
                            time.sleep(char_delay)

                            # Type the correct character
                            vk_code = VK_CODES.get(char, VK_CODES.get(char.lower(), 0))
                            if self.validate_vk_code(vk_code) and self.simulate_keypress(vk_code, char):
    ...
    def simulate_random_word(self):
        ...
        for _ in range(word_length):
            # SAFETY: Verify window is still valid
            if not self.is_window_valid():
                self.logger.warning("Window became invalid during typing")
                return False
    ...
    def simulate_sentence(self):
        ...
        SAFETY: Validates window throughout sentence typing.
        ...
        # SAFETY: Verify window is valid before starting
        if not self.is_window_valid():
            return False
        ...
        if self.validate_vk_code(vk_code) and self.simulate_keypress(vk_code, punctuation):
    ...
    def simulate_code_snippet(self):
        ...
        SAFETY: Validates window and key codes throughout.
        ...
        for char in code:
            ...
            if char == '\n':
                ...
                try:
                    ...
                    self.logger.warning(f"Error getting vk code for char '{char}': {e}")
                    continue
    ...
    def simulate_number_sequence(self):
        ...
        SAFETY: Validates window and key codes.
        ...
        for _ in range(length):
            ...
            if self.validate_vk_code(vk_code) and self.simulate_keypress(vk_code, digit):
    ...
    def simulate_special_key(self):
        ...
        SAFETY: Validates window and key code.
        ...
        if self.validate_vk_code(vk_code) and self.simulate_keypress(vk_code):
    ...
    def cleanup_window(self):
        ...
        with self.window_lock:
            ...
            if self.transparent_window:
                try:
                    if self.bounds_validator.is_window_valid(self.transparent_window):
...
if __name__ == "__main__":
    ...
    print(f"SafeKeyboardTester v2.0 - Test keyboard input in an isolated environment")
